(function(){
  ...
  function draw(){
    ...
    var s={ 'kcal-asc':function(a,b){return a.kcal-b.kcal;}, 'kcal-desc':function(a,b){return b.kcal-a.kcal;}, protein:function(a,b){return b.protein-a.protein;}, time:function(a,b){return a.time-b.time;} }[st.sort];
  ...
  function card(r){
    return '<button type="button" class="rcard" data-id="'+r.id+'"><span class="rband '+r.category+'"></span><span class="rbody">'+
      '<span class="rmeta"><span>'+F.CATS[r.category]+'</span><span>'+r.time+' dk</span></span>'+
      '<h3>'+esc(r.title)+'</h3><p>'+esc(r.desc)+'</p>'+
      '<span class="badges">'+(r.fit?'<span class="badge fit">Fit</span>':'')+r._plans.map(function(p){return '<span class="badge">'+F.PLANS[p].name+'</span>';}).join('')+'</span>'+
      '<span class="rkcal"><span class="num">'+fmt(r.kcal)+'</span><small>kcal / porsiyon</small></span>'+
      '<span class="rmac"><span>P '+r.protein+' g</span><span>K '+r.carbs+' g</span><span>Y '+r.fat+' g</span></span>'+
      '</span></button>';
  }
  ...
  function openRecipe(id){
    ...
    $('dBody').innerHTML=
      ...
      '<div class="rd-stats"><div><span>Porsiyon kalorisi</span><strong>'+fmt(r.kcal)+' kcal</strong></div><div><span>Protein</span><strong>'+r.protein+' g</strong></div><div><span>Karbonhidrat</span><strong>'+r.carbs+' g</strong></div><div><span>Yağ</span><strong>'+r.fat+' g</strong></div></div>'+
